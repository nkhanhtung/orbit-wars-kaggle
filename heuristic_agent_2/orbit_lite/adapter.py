...
import torch
from .constants import P_MAX, F_MAX, COMET_EVENTS, COMETS_PER_EVENT, COMET_PATH_MAX
...
def _infer_player_count_from_obs(planets: list[Any], fleets: list[Any], player_id: int) -> int:
    """Suy luận số người chơi từ observation thô.

Ưu tiên metadata nếu có, nếu không thì đọc owner trong danh sách hành tinh ban đầu. Hàm giúp agent phân biệt trận 2 người và 4 người để projection garrison có đúng số kênh người chơi."""
...
def _dict_obs_to_tensor(obs: dict[str, Any], player_id: int, P: int=P_MAX, F: int=F_MAX, device: Any='cpu') -> dict[str, Any]:
    """Chuyển observation dạng dict/list của Kaggle thành tensor nội bộ.

Thuật toán chuẩn hoá planet, fleet, initial_planets và metadata về cùng device/dtype. Adapter này cô lập sự khác biệt format I/O, giúp planner chỉ làm việc với tensor ổn định."""
...
def _sparse_actions_to_list(action_payload: dict[str, Any], obs: dict[str, Any], player_id: int) -> list[list[Any]]:
    """Đổi action sparse nội bộ thành danh sách move Kaggle.

Hàm lọc dòng action hợp lệ, lấy source, angle, ships và ép kiểu về Python number để Kaggle environment có thể serialize."""
...
def single_obs_to_tensor(obs: dict[str, Any], *, player_id: int, P: int=P_MAX, F: int=F_MAX, device: Any='cpu') -> dict[str, Any]:
    """API công khai để chuẩn hoá một observation đơn lẻ.

Hàm gọi adapter dict-to-tensor, bảo toàn player_id và đưa dữ liệu về đúng device để các module Torch phía sau xử lý thống nhất."""
...
def sparse_action_row_to_moves(action_payload: dict[str, Any], obs: dict[str, Any], *, player_id: int) -> list[list[Any]]:
    """API công khai chuyển một hàng action sparse thành move list.

Được gọi ở cuối agent để trả về format [[source_id, angle, ships], ...] theo yêu cầu submission."""
